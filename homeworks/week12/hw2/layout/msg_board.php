<?php
  ...
  function moreOption($row){
    global $certificate, $auth;
    if ($row['certificate'] === $certificate || $auth === 'admin' || $auth === 'super_admin') {
  ...
  function createReplyForm($row_reply, $layer = 0) {
    global $csrfToken, $row;
...
<div class="msgboard">
  ...
  <?php
    // SQL 指令，查詢留言列表
    $offset = $page * 20 - 20;
    $stmt = $conn->prepare("SELECT comments.*, avatar, nickname, authority, certificate
    ...
    $sql_reply = "SELECT comments.*, avatar, nickname, authority, certificate
                  FROM zihur_comments AS comments
                  JOIN zihur_users AS users
                    ON comments.user_id = users.id
                  LEFT JOIN zihur_users_certificate AS certificate
  ...
  <section class="msgcard">
    ...
    <div class="msgcard__inside">
      ...
      <?php
        moreOption($row_reply);//                                 <= 比對並叫出編輯刪除等按鈕
        userThumb($row_reply, $user_id, $conn);//                 <= 比對並叫出按讚按鈕
        createReplyForm($row_reply, $layer);//                    <= 建立回文表單
      ?>
    </div>
    <?php
        endif;
